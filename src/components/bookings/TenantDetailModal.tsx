...
import { FileText, Phone, MessageCircle, Calendar, DollarSign, Home } from 'lucide-react';
          ...
          <div className="space-y-3">
            ...
            <div className="rounded-lg border p-4 space-y-3">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-medium">{formatEGP(booking.daily_rate * booking.duration_days)}</span>
              </div>
              {booking.deposit_amount > 0 && (
                <div className="flex justify-between items-center">
                  <span className="text-muted-foreground">{t('deposit')}</span>
                  <span className="font-medium">{formatEGP(booking.deposit_amount)}</span>
                </div>
              )}
              {booking.housekeeping_amount > 0 && (
                <div className="flex justify-between items-center">
                  <span className="text-muted-foreground">{t('housekeeping')}</span>
                  <span className="font-medium">{formatEGP(booking.housekeeping_amount)}</span>
                </div>
              )}
              <div className="border-t pt-3 flex justify-between items-center">
                <span className="font-semibold">{t('totalAmount')}</span>
                <span className="font-bold text-lg text-primary">{formatEGP(booking.total_amount)}</span>
              </div>
            </div>
